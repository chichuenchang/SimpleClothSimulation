...
__global__ void test_simple_vbo_kernel(glm::vec1* pos, unsigned int width, unsigned int height, float time)
{
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x > width || y > height) return;
    ...
    //pos[x * height + y][1] = w;
    //pos[x * height + y][2] = v;
    pos[(x * height + y)* 5 + 0] = glm::vec1(u);
    pos[(x * height + y)* 5 + 1] = glm::vec1(w);
    pos[(x * height + y)* 5 + 2] = glm::vec1(v);

}

void test_launch_kernel(glm::vec1* pos, const unsigned int mesh_width,
    const unsigned int mesh_height, float time)
{
    // execute the kernel
    dim3 block(8, 8, 1);
